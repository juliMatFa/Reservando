...
Listado.prototype.buscarRestaurante = function(identificador) {
    var restaurant = this.restaurantes.find(restaurante => restaurante.id === identificador);
...
//Borra repetidos
Listado.prototype.borrarElementoRepetido = function(array){
    var listado = array.filter(function(elem, index, self) {
...
Listado.prototype.obtenerUbicaciones = function() {
    ...
    var ciudadesNoRepetidas = this.borrarElementoRepetido(ciudades);
    return ciudadesNoRepetidas.sort();
}
...
Listado.prototype.obtenerRubros = function() {
    ...
    var rubrosNoRepetidos = this.borrarElementoRepetido(rubros);
    return rubrosNoRepetidos.sort();
}


    //Obtiene todos los horarios de los restaurantes (sin repetidos).
    Listado.prototype.obtenerHorarios = function () {
        //Array en que se van a cargar los arrays de horarios, que luego vamos convertir en un solo array
        var arrayHorarios = [];
        ...
        for (var i = 0; i < this.restaurantes.length; i++) {
            arrayHorarios.push(this.restaurantes[i].horarios);
        }

        //En este arreglo vamos a poner todos los horarios, uno por uno
        var listaHorarios = [];
        arrayHorarios.forEach(function (arrayHorarios) {
            arrayHorarios.forEach(function (hor) {
                listaHorarios.push(hor)
            });
        });

        //En este arreglo vamos a poner todos los horarios pero sin repetidos
        var horariosSinRepetir = listaHorarios.filter(function (elem, index, self) {
            return index === self.indexOf(elem);
        });
